 ...
 *
 * **按类型分流（v2.39 / v2.41）**：
 * · **骑缝章** —— 自 v2.39 起**仅往来函证检测**（R-03），银行侧**不渲染**该行；
 * · **结论条** —— 往来侧印章落章区域**决定**相符性（落「信息不符」区 → 回函结果应为不相符）；
 *   银行侧**印章不参与相符性判定**（R-01：只由「询证事项逐项核对」决定），故结论条只讲印章本身，
 *   不讲"回函结果应判为什么"。
 */
export default function SealRecognition({
  seal,
  entity,
  /** 银行函证 —— 不渲染骑缝章行、结论条也不推导相符性 */
  isBank = false,
}: {
  seal: SealResult
  entity: string
  ...
  /* ① 结论：一句话讲清「印章怎么样」，不复述业务规则 */
  const status = !seal.hasSeal || regionIsMismatch ? 'risk' : 'ok'
  const conclusion = isBank
    ? !seal.hasSeal
      ? '未检出印章 · 建议退回被询证方补盖银行印章'
      : seal.nameMatched
        ? '已盖章，印章名称与被询证单位一致'
        : '印章名称与被询证单位不一致 · 建议核实'
    : !seal.hasSeal
  ...
  return (
    <div>
      ...
      <div style={{ background: 'var(--c-fill-light)', borderRadius: 8, padding: '8px 12px' }}>
        ...
        />
        {/* 骑缝章：仅往来函证检测（v2.39 起 R-03 限定往来；银行回函无此业务要求） */}
        {!isBank && (
          <InfoRow
            label="骑缝章"
        ...
        <InfoRow
          ...
          hint={isBank ? '仅作事实记录 —— 银行函证的相符性判定不看落章区域' : '决定回函结果是否相符'}
          confidence={0.92}
        />
      </div>
